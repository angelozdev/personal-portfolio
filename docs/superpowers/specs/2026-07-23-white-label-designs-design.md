...
| Decisión | Elección |
| --- | --- |
| Marcas viejas (ember/tide/bloom) | Se eliminan. Quedan 2 diseños: AZ (actual) y Brutal (nuevo) |
| Estética del segundo diseño | Brutalismo crudo |
| Transición de cambio | View Transitions API con fallback instantáneo |
| Arquitectura | A+: contrato de tokens rico + `base.css` opcional restringido a elementos semánticos |
...
src/designs/
  ...
  index.ts        ← registry: [az, brutal] + defaultDesign ("az")
  az/
    manifest.ts   ← label "AZ"; imports fontsource de IBM Plex (side-effect)
  brutal/
    manifest.ts   ← label "Brutal"; imports fontsource propios
    tokens.css    ← [data-design="brutal"] { redefine el contrato completo }
    base.css      ← [data-design="brutal"] solo elementos: textura body, ::selection, scrollbar
```

### Reglas del sistema

1. **El contrato son los tokens base** (`src/styles/tokens/*.css`). Los valores default de `:root` SON el diseño AZ. Un diseño alterno hace override bajo `[data-design="<id>"]`. AZ no duplica nada.
...
3. **Los componentes conocen solo el contrato.** Nunca referencian un diseño. Un diseño nunca referencia una clase de componente: `base.css` se restringe a selectores de elementos semánticos (`body`, `::selection`, scrollbar, `h1–h6`) y atributos data. El HTML semántico es API estable; las clases BEM no.
4. **Agregar un diseño #3** = crear carpeta con `manifest.ts` + `tokens.css` (+ `base.css` opcional) y agregar una línea en `index.ts`. Nada más.
...
`@font-face` es lazy: el browser solo descarga los woff2 del diseño activo. El preload en `<head>` cubre solo las fuentes del diseño default (AZ), como hoy. Al activar Brutal por primera vez hay FOUT breve; aceptable.

## Contrato de tokens expandido

Tokens nuevos (defaults = AZ actual; cero cambio visual en AZ):

- **Tipografía:** `--font-display` (AZ: IBM Plex Mono), `--font-body` (AZ: IBM Plex Sans), `--heading-transform` (AZ: `none`), `--heading-tracking` (AZ: `var(--tracking-tight)`). `--font-sans`/`--font-mono` quedan como primitivas.
- **Bordes:** `--border-width` (AZ: `1px`), `--border-width-thick` (AZ: `2px`).
...
- **base.css:** `::selection` amarillo/negro, scrollbar cuadrada gruesa, textura sutil de papel cuadriculado en `body`.
...
- **Test de contrato:** verifica que `brutal/tokens.css` define todos los tokens de la lista del contrato. Un diseño futuro que olvide un token rompe el test — el contrato se auto-vigila.
...
1. AZ se ve idéntico a hoy (regresión visual cero).
2. Brutal se percibe como un producto distinto: otra tipografía, otro color, otra textura, otro motion — mismo layout.
